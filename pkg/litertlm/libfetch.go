...
// FetchLib explicitly downloads and caches the LiteRT-LM prebuilt shared libraries
...
func FetchLib(os, platform, version string) (string, error) {
	targetPlatform, err := libfetch.PlatformFor(os, platform)
	if err != nil {
		return "", fmt.Errorf("litertlm: FetchLib: %w", err)
	}
